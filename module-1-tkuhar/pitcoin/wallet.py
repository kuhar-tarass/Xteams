...
class k_box:
    ...
    def get_public_key_compresed(self):
        return (0x03.to_bytes(1,byteorder=byteorder) if self.__pub_key[-1] % 2 else 0x02.to_bytes(1,byteorder=byteorder)) + self.__pub_key[0:32]
    ...
    def get_encrypted_pub_key(self):
        sha = hashlib.sha256(self.get_public_key()).digest()
...
def base58_decode(s:str, len)->bytearray:
    alphabet = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'
    result = 0
    for c in s:
        result = result * 58 + alphabet.index(c)
    return result.to_bytes(len,byteorder="big")
